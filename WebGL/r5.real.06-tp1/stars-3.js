...
in vec2 position;
uniform mat2 matrice2D;

void main() {
  vec2 position_rotated = matrice2D * position.xy;
  gl_Position = vec4(position_rotated, 0.0, 1.0);
}
`;
...
const matrix_uniform = gl.getUniformLocation(prg, "matrice2D");
...
gl.uniformMatrix2fv(matrix_uniform, true, matRotation);
...
function animate() {
  ...
  angle += 0.05;
  matRotation[0] = Math.cos(angle);
  matRotation[1] = Math.sin(angle);
  matRotation[2] = -Math.sin(angle);
  matRotation[3] = Math.cos(angle);
  gl.uniformMatrix2fv(matrix_uniform, true, matRotation);
  gl.drawArrays(gl.TRIANGLES, 0, points.length / 2);
